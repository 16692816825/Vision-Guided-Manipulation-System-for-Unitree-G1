...
# 抓住瓶子并抬小臂后，先让大臂外扩，再下放小臂和松手。
# 这里只改 16 号肩横滚，避免覆盖最新的小臂高度和抓取点参数。
...
class StandalonePickPlace:
    ...
    def build_targets(self, q0):
        ...
        self.q_lift_bottle_out = dict(self.q0)
        self.q_release_out = dict(self.q0)
        ...
        self.q_lift_bottle_out = dict(self.q_lift_bottle)
        self.q_lift_bottle_out[16] = self.q0[16] + OUTWARD_RELEASE_SHOULDER_ROLL_DELTA
        self.q_release_out = dict(self.q_unfold_pregrasp)
        self.q_release_out[16] = self.q0[16] + OUTWARD_RELEASE_SHOULDER_ROLL_DELTA
        ...
        for j in ALL_ARM:
            print(
                ...
                "lift_bottle_out =", round(self.q_lift_bottle_out[j], 4),
                "release_out =", round(self.q_release_out[j], 4),
    ...
    async def run(self):
        ...
        try:
            ...
            self.phase("pick: move upper arm outward with lifted bottle", 2.0, self.q_lift_bottle, self.q_lift_bottle_out, 1.0, 1.0)
            self.phase("pick: lower forearm at outward release position", 2.0, self.q_lift_bottle_out, self.q_release_out, 1.0, 1.0)

            self.phase("place: hold before release", 0.5, self.q_release_out, self.q_release_out, 1.0, 1.0)
            await self.send_hand_pose("open", 2.0)
            self.phase("place: hold after release", 1.0, self.q_release_out, self.q_release_out, 1.0, 1.0)

            self.phase("place: fold forearm back after release", 3.0, self.q_release_out, self.q_lift_folded, 1.0, 1.0)
