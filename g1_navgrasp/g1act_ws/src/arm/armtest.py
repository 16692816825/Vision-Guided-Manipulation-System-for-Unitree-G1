...
class Custom:
    def __init__(self):
        ...
        self.transition_time = 2.0       # 动作1：抬臂时间
        self.hold_time = 15.0            # 动作1：保持时间
        self.transition2_time = 2.0      # 动作2：伸直手臂过渡时间
        self.hold2_time = 5.0           # 动作2：前伸保持时间
        self.release_time = 3.0          # 释放时间
        ...
        self.left_target_pos = {
            G1JointIndex.LeftShoulderPitch: -0.7,
            G1JointIndex.LeftShoulderRoll:  -0.1,
            G1JointIndex.LeftShoulderYaw:   -0.3,
            G1JointIndex.LeftElbow:         -0.5,
            G1JointIndex.LeftWristRoll:     -0.6,
        }

        # --------------------------
        # 动作2：手臂向前伸直 + 手掌朝外（你要的新姿势）
        # --------------------------
        self.left_forward_target = {
            G1JointIndex.LeftShoulderPitch: -1.2,   # 手臂向前水平伸直
            G1JointIndex.LeftShoulderRoll:  0.5,   # 自然朝前
            G1JointIndex.LeftShoulderYaw:   0.2,
            G1JointIndex.LeftElbow:         -0.3,    # 手臂完全伸直
            G1JointIndex.LeftWristRoll:     1.5,    # 手掌向外/向前
        }
    ...
    def LowCmdWrite(self):
        ...
        if self.time_ < self.transition_time:
            ...
            for jid in self.right_init_pos:
                self._set_joint_target(jid, self.right_init_pos[jid])
            for jid in self.waist_init_pos:
        ...
        # ------------------------------------
        # 阶段3：平滑过渡 → 手臂向前伸直、手掌朝外
        # ------------------------------------
        elif self.time_ < self.transition_time + self.hold_time + self.transition2_time:
            t_start = self.transition_time + self.hold_time
            ratio = (self.time_ - t_start) / self.transition2_time
            for jid in self.left_arm_joints:
                start_q = self.left_target_pos.get(jid, self.left_init_pos[jid])
                target_q = self.left_forward_target.get(jid, start_q)
                q_des = (1.0 - ratio) * start_q + ratio * target_q
                self._set_joint_target(jid, q_des)
            for jid in self.right_init_pos:
                self._set_joint_target(jid, self.right_init_pos[jid])
        ...
        # 阶段4：保持手臂前伸、手掌朝外姿势
        # ------------------------------------
        elif self.time_ < self.transition_time + self.hold_time + self.transition2_time + self.hold2_time:
            for jid in self.left_arm_joints:
                target_q = self.left_forward_target.get(jid, self.left_init_pos[jid])
                self._set_joint_target(jid, target_q)
            for jid in self.right_init_pos:
                self._set_joint_target(jid, self.right_init_pos[jid])
            for jid in self.waist_init_pos:
            ...
            elapsed = self.time_ - (self.transition_time + self.hold_time + self.transition2_time)
            if int(elapsed * 10) % 10 == 0:
                print(f"手臂前伸、手掌朝外保持中：{elapsed:.1f} / {self.hold2_time:.0f}s")

        # ------------------------------------
        # 阶段5：柔顺释放
        # ------------------------------------
        elif self.time_ < self.transition_time + self.hold_time + self.transition2_time + self.hold2_time + self.release_time:
            release_progress = (self.time_ - (self.transition_time + self.hold_time + self.transition2_time + self.hold2_time)) / self.release_time
