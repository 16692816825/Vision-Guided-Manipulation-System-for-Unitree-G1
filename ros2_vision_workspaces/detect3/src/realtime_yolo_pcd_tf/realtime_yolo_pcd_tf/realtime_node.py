...
class RealtimeYoloPcdTfNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('z_max_m', 6.0)
        self.declare_parameter('min_valid_ratio', 0.05)
        self.declare_parameter('min_valid_points', 5000)
        self.declare_parameter('reuse_last_cloud_on_bad_frame', True)

        self.declare_parameter('extrinsics_json', '/home/xinghg/detect/robot_center_to_camera.json')
        ...
        self._last_good_cloud: Optional[PointCloud2] = None
    ...
    def _publish_tf(self):
        ...
        if not self._static_tf_sent:
            self._static_tfb.sendTransform(ts)
    ...
    def _imgmsg_from_bgr(self, frame_id: str, img_bgr, stamp_msg=None) -> Image:
        msg = Image()
        msg.header.stamp = stamp_msg if stamp_msg is not None else self.get_clock().now().to_msg()
    ...
    def _build_cloud(self, frame_id: str, color_bgr, depth_m, highlight_bbox, stamp_msg=None):
        import numpy as np
        ...
        valid = (z > float(z_min)) & (z < float(z_max)) & np.isfinite(z)
    ...
    def _worker_loop(self):
        ...
        while not self._stop_event.is_set():
            ...
            # Use sensor timestamp for consistency with TF and RViz.
            stamp_msg = depth_msg.header.stamp
            self.pub_rgb.publish(self._imgmsg_from_bgr(camera_frame, vis, stamp_msg=stamp_msg))

            cloud = self._build_cloud(camera_frame, color_bgr, depth_m, highlight_bbox, stamp_msg=stamp_msg)
            if cloud is not None:
                min_ratio = float(self.get_parameter('min_valid_ratio').value)
                min_pts = int(self.get_parameter('min_valid_points').value)
                reuse_last = bool(self.get_parameter('reuse_last_cloud_on_bad_frame').value)

                total_pts_est = (int(depth_m.shape[0]) // max(1, int(self.get_parameter('cloud_stride').value))) * (
                    int(depth_m.shape[1]) // max(1, int(self.get_parameter('cloud_stride').value))
                )
                valid_ratio = float(cloud.width) / float(max(1, total_pts_est))

                if cloud.width < max(0, min_pts) or valid_ratio < max(0.0, min_ratio):
                    self._publish_status(
                        f"bad_depth_frame drop cloud: valid_pts={int(cloud.width)} ratio={valid_ratio:.3f}"
                    )
                    if reuse_last and self._last_good_cloud is not None:
                        self.pub_cloud.publish(self._last_good_cloud)
                else:
                    self._last_good_cloud = cloud
